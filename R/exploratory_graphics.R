...
#'   \code{psmelt()} or \code{make_phy_df()}
#' @param vars (\code{NULL}) A character vector of grouping variables from which
#'   the baseline values are chosen to define the abundance ordering. If it is
#'   \code{NULL}, the ordering will be based on mean abundances in the whole
#'   data frame.
...
#' @param ranks
#'   (\code{c('Kingdom','Phylum','Class','Order','Family','Genus','Species')}) A
#'   character vector of taxomonic ranks used by your data set.
rank_abund = function(phy_df, vars = NULL, bases = NULL, abund = 'Abundance',
    ...
    } else if (length(vars) != length(bases)){
        stop('vars and bases must have the same length.')
    }

    # Subset for ordering
    new_df = phy_df
    if (!is.null(vars) & !is.null(bases)){
        for (i in 1:length(vars)){
            var = UQ(sym(vars[i]))
            base = bases[i]
            new_df = filter(new_df, var == base)
        }
    }
    ...
    lev_lst = list()
    for (rank in ranks){
        new_df[,rank] = order_taxa(new_df, rank, abund, decreasing = TRUE)
        lev_lst[[rank]] = levels(new_df[,rank])
        # Check for 'Other' in original data set
        if (!('Other' %in% phy_df[,rank])){
            lev_lst[[rank]] = lev_lst[[rank]][-1]   # Remove 'Other' (it's
                                                    # always first)
        }
    }

    # Add any levels that weren't in the subsetted data frame back to the levels
    # And give the data frame its new levels
    for (rank in ranks){
        missing = levels(phy_df[,rank])
        missing = missing[!(missing %in% lev_lst[[rank]])]
        lev_lst[[rank]] = c(lev_lst[[rank]],missing)
        phy_df[,rank] = factor(phy_df[,rank], levels = lev_lst[[rank]])
    }
